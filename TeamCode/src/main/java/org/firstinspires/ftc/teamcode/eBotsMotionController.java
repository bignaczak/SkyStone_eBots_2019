...
public class eBotsMotionController {
    ...
    //TODO:  This method should either be moved to the Auton abstract class, and move from static to class method,
    // removing almost all parameters or turn into an implemented method and pass a parameters object to it
    ...
    //TODO:  This should be broken up into smaller routines for each permutation of the drive vector
    public static void activateDriveMotors(TrackingPose trackingPose, double xSig, double ySig, double spinSig, Speed speed, SoftStart softStart, StopWatch travelLegTimer, Telemetry telemetry) {
        ...
        double robotAngleRad = travelDirectionRad - robotHeadingRad + Math.PI / 4;

        //  Calculate the driveValues, ignoring spin for first pass
        double[] driveValues = new double[4];

        driveValues[0] = Math.cos(robotAngleRad) * translateSig;
        driveValues[1] = Math.sin(robotAngleRad) * translateSig;
        driveValues[2] = Math.sin(robotAngleRad) * translateSig;
        driveValues[3] = Math.cos(robotAngleRad) * translateSig;
    ...
    //TODO:  This should be combined with moveToTargetPose.  The primary difference is how the exit
    // conditions are checked.  This one only uses time, while the other one uses time and acheived pose,
    // which could be handled as an argument
    public static void defendPosition(TrackingPose trackingPose, Long duration, Speed speed, GyroSetting gyroSetting, Accuracy accuracy, SoftStart softStart, BNO055IMU imu, Telemetry telemetry){
        /*
        1) Calculate x,y, spin components of error
        2) Calculate direction for travel
        3) Calculate x,y, spin components for Raw Drive intent (raw because can exceed motor limits)
        ...
        final boolean debugOn = true;
        final String logTag = "BTI_defendPosition";
        double debugIncrement = 1.0;
